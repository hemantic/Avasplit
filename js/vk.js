...
function vk_cut(cut_function) { // Вспомогательная функция, отвечает за получение информации о серверах VK
  VK.Api.call('photos.createAlbum', {title: 'avasplit', privacy: 0, comment_privacy: 3, description: ''}, function(r){ // создаем альбом
    if(r.response) {
      vk_aid = r.response.aid; // передаём частные значения глобальным переменным
      VK.Api.call('photos.getUploadServer', {aid: vk_aid}, function(r){
        if(r.response) {
